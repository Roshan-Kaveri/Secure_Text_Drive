            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if response_data.get("email_found") == "true":
                        flash('OTP sent to your email!', 'error')
                        return render_template('forgot_pass.html',error="Email send")

                    else:
                        # Email not found in the database
                        flash('OTP sent to your email!', 'success')
                        #return redirect(url_for('views.homes'))
                        return render_template('forgot_pass.html',error="No email found")
                else:
                    error_data = response.json()
...
def download_file(filename):
    ...
    decrypted_content = file_content    # = decrypt(binascii.unhexlify(file_content),public_key)
    print(file_data , "\n))", decrypted_content)
